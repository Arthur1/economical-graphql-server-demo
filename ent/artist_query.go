...
type ArtistQuery struct {
	...
	withNamedSongs map[string]*SongQuery
...
func (aq *ArtistQuery) QuerySongs() *SongQuery {
	query := (&SongClient{config: aq.config}).Query()
...
func (aq *ArtistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Artist, error) {
	...
	if query := aq.withSongs; query != nil {
		if err := aq.loadSongs(ctx, query, nodes,
			func(n *Artist) { n.Edges.Songs = []*Song{} },
			func(n *Artist, e *Song) { n.Edges.Songs = append(n.Edges.Songs, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedSongs {
		if err := aq.loadSongs(ctx, query, nodes,
			func(n *Artist) { n.appendNamedSongs(name) },
			func(n *Artist, e *Song) { n.appendNamedSongs(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *ArtistQuery) loadSongs(ctx context.Context, query *SongQuery, nodes []*Artist, init func(*Artist), assign func(*Artist, *Song)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(song.FieldArtistID)
	}
...
// WithNamedSongs tells the query-builder to eager-load the nodes that are connected to the "songs"
...
func (aq *ArtistQuery) WithNamedSongs(name string, opts ...func(*SongQuery)) *ArtistQuery {
	...
	if aq.withNamedSongs == nil {
		aq.withNamedSongs = make(map[string]*SongQuery)
	}
	aq.withNamedSongs[name] = query
	return aq
}
